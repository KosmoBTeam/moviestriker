<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<footer class="css-elo5ab-Self eyuxr3e0"
	style="position: relative; background: rgba(0, 0, 0, .5); padding: 27px 50px 40px; display: block;">
	<ul class="css-d9xhzx-LegalAndHelps eyuxr3e12">
		<li class="css-15wgbxm-LegalAndHelpList eyuxr3e11">무비스트라이커 서비스
			이용약관</li>
		<li class="css-1yd5as9-LegalAndHelpList eyuxr3e11">개인정보 처리 방침</li>
		<li class="css-15wgbxm-LegalAndHelpList eyuxr3e11"><a
			class="css-pz9ouq-HelpLink eyuxr3e9" href="/help">고객센터</a></li>
		<li class="css-15wgbxm-LegalAndHelpList eyuxr3e11"><a
			href="https://team.watcha.com/joinus" target="_blank"
			rel="noreferrer noopener" class="css-1nrmw3u-Href eyuxr3e10">채용정보</a></li>
	</ul>
	<ul class="css-ixy3be-ContactInfo eyuxr3e1">
		<li class="css-1cqje1p-ContactInfoList eyuxr3e2"><span
			class="css-suh0oq-ContactInfoListTitle eyuxr3e4">고객센터(이용 및 결제
				문의)</span><span class="css-1lm2916-ContactInfoListDescription eyuxr3e3"><a
				href="mailto:cs@watcha.co.kr" class="css-1nrmw3u-Href eyuxr3e10">gusrldlqslek@gmail.com</a>
				/010-3332-3266 (유료)</span></li>
		<li class="css-1cqje1p-ContactInfoList eyuxr3e2"><span
			class="css-suh0oq-ContactInfoListTitle eyuxr3e4">제휴 및 대외 협력</span><span
			class="css-1lm2916-ContactInfoListDescription eyuxr3e3"><a
				href="mailto:contact@watcha.com" class="css-1nrmw3u-Href eyuxr3e10">gusrldlqslek@naver.com</a>
				/010-3332-3266 (유료)</span></li>
	</ul>
	<div class="css-2x8q7u-ExtraBlock eyuxr3e6">
		<ul class="css-1jweooo-ExtraInfo eyuxr3e7">
			<li class="css-n0i3qz-ExtraInfoList eyuxr3e8">주식회사 무비스트라이커</li>
			<li class="css-n0i3qz-ExtraInfoList eyuxr3e8">대표 이현기</li>
			<li class="css-n0i3qz-ExtraInfoList eyuxr3e8">서울특별시 금천구 가산동
				426-5 월드 메르디앙 벤처 센터 2 차 410 호</li>
		</ul>
		<ul class="css-1jweooo-ExtraInfo eyuxr3e7">
			<li class="css-n0i3qz-ExtraInfoList eyuxr3e8">사업자등록번호
				321-2344-1134</li>
			<li class="css-n0i3qz-ExtraInfoList eyuxr3e8">통신판매업 신고번호 제
				2019-서울금천-0965호</li>
		</ul>
	</div>
	<div class="css-ui4zr5-SnsLinks eyuxr3e13">
		<a
			src="data:image/svg+xml;base64,"
			href="https://fb.com/watchaKR" target="_blank"
			rel="noreferrer noopener" class="css-wxj7x7-SnsLink eyuxr3e14"></a><a
			src="data:image/svg+xml;base64,"
			href="https://twitter.com/watcha_kr" target="_blank"
			rel="noreferrer noopener" class="css-11ty0gt-SnsLink eyuxr3e14"></a><a
			src="data:image/svg+xml;base64,"
			href="https://www.instagram.com/watcha_kr/" target="_blank"
			rel="noreferrer noopener" class="css-a6x8s2-SnsLink eyuxr3e14"></a><a
			src="data:image/svg+xml;base64,"
			href="https://team.watcha.com" target="_blank"
			rel="noreferrer noopener" class="css-18div9m-SnsLink eyuxr3e14"></a>
	</div>
	<div class="css-8mx6ne-Copyright eyuxr3e5">
		<a
			href="https://watcha.team/WATCHA%200a7284a6c7224e939be9e8f4af5e8be0/Untitled%20b095476de108404ba618447de1b20594.html"><img
			src="" width="55" alt="MovieStriker"
			class="css-4mavcu-FooterLogo eyuxr3e15"></a>Copyright © 2020 by <em>MovieStriker,
			Inc.</em> All rights reserved.
	</div>
</footer>
<script type="text/javascript">
	function searchToggle(obj, evt) {
		var container = $(obj).closest('.search-wrapper');
		if (!container.hasClass('active')) {
			container.addClass('active');
			evt.preventDefault();
		} else if (container.hasClass('active')
				&& $(obj).closest('.input-holder').length == 0) {
			container.removeClass('active');
			// clear input
			container.find('.search-input').val('');
		}
	}
	$(document).ready(
			function() {
				if($('#myid').val()==''){
					window.location.replace("http://localhost:8080/moviestriker/goLogin");
					}
				$("main").on(
						"click",
						function(e) {
							/* 							var windowWidth = $(window).width();

							 alert(windowWidth); */
							if ($('.search-wrapper').hasClass("active")
									&& $('#holder').width() == 250) {
								$('.search-wrapper').removeClass('active');
								// clear input
								$('.search-wrapper').find('.search-input').val(
										'');
							}

						});
				$('.toggledown').mouseover(function() {
					console.log('down');
					$('#togglediv1').stop().show();
				});
				$('.toggledown').mouseleave(function() {
					console.log('up');
					$('#togglediv1').stop().hide();
				});
				$.fn.scrollStopped = function(callback) {
					var that = this, $this = $(that);
					$this.scroll(function(ev) {
						clearTimeout($this.data('scrollTimeout'));
						$this.data('scrollTimeout', setTimeout(callback
								.bind(that), 250, ev));
					});
				};
				$('body').mouseover(function(e) {
					if ($(e.target).hasClass("fa-list-ul") || $(e.target).hasClass("displaynone")|| $(e.target).hasClass("masthead")|| $(e.target).hasClass("toli")) {
						$('#togglediv2').stop().show();
					} else {
						$('#togglediv2').stop().hide();
					}
				});
				/* 				$('.fa-list-ul').mouseover(function() {
				
				 });
				 $('.fa-list-ul').mouseleave(function() {
				
				 }); */
				var lastScrollTop = 0;
				var delta = 5;
				var alpha = 20; // 동작의 구현이 시작되는 위치 
				var navbarHeight = $('header').outerHeight();
				var st = $(this).scrollTop();
				var didScroll;
				$(window).scroll(function(event) {
					didScroll = true;
				});
				setInterval(function() {
					if (didScroll) {
						hasScrolled();
						didScroll = false;
					}
				}, 250);
				function hasScrolled() {

					var st = $(this).scrollTop();

					if (st > 5) {
						$('header').removeClass('nav-down').addClass('nav-up');
					} else {
						$('header').removeClass('nav-up').addClass('nav-down');
					}

					lastScrollTop = st;

				}

				$("body").on("click", function(e) {
					if ($(e.target).hasClass("fa-list-ul")) {

					} else if ($(e.target).hasClass("masthead")) {

					} else {

					}
				});

			});
</script>
